...
The Final Configuration Specification is a roll up of all the sequence of package handler items.  Later package handler items can change the properties of earlier package handler item configuration properties, resulting in the Final Configuration Specification.

It is this rolling up of all sequential changes to the specification that gives sysync the propery of [idempotency](http://en.wikipedia.org/wiki/Idempotence).  It compares the state of the host to the ideal state of the configuration, and only makes the changes necessary to correct the inconsistencies, without doing extraneous work such as putting a file there and then removing it later, even if the Handler items that describe the configuration appear in that order.
