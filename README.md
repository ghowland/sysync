...
The second template data is "backup_server", and it is equal to a list.  This means that the values in the list will be selected by means of the number of items (3), and the position of the host in the host list (0).  The algorithm would then be:  0 % 3.  Zero modulus three equals zero (0), so the first item in the list is choosen: "backup-1"
